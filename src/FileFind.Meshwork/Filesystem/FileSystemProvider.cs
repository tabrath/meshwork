...
namespace FileFind.Meshwork.Filesystem
{
	...
	public class FileSystemProvider
	{
			...
			if (!create) {
				try {
					...
					if (RootDirectory == null) {
						Core.LoggingService.LogWarning("Unable to find root dir");
						create = true;
					} else {
						// Verify version
						string currentVersion = ExecuteScalar("SELECT value FROM properties WHERE name='version'").ToString();
						if (currentVersion != SCHEMA_VERSION) {
							Core.LoggingService.LogWarning("Schema has changed, recreating db.");
							create = true;
						}
					}
		...
		{
			Core.LoggingService.LogDebug("ExecuteDataSet: {0}", GetCommandTextWithParameters(command));
		...
		public object ExecuteScalar(string query)
		{
			...
			UseConnection(delegate (IDbConnection connection) {
				...
				Core.LoggingService.LogDebug("ExecuteScalar: {0}", GetCommandTextWithParameters(cmd));
		...
		{
			Core.LoggingService.LogDebug("ExecuteScalar: {0}", GetCommandTextWithParameters(command));
			return command.ExecuteScalar();
		}
		
		public int ExecuteNonQuery(IDbCommand command)
		{
			Core.LoggingService.LogDebug("ExecuteNonQuery: {0}", GetCommandTextWithParameters(command));
		...
		internal void ProcessFileDetailsMessage(Network network, Node messageFrom, SharedFileListing info)
		{
            ...
            FileFind.Meshwork.FileTransfer.IFileTransfer transfer = Core.FileTransferManager.Transfers.SingleOrDefault(t => t.File == remoteFile);
